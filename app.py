...
def render_recommendation_details(risk_info, recommendation_text, is_upf, upf_flags):
    # PERUBAHAN Teks & Font: Rekomendasi
...
def render_holistic_nutrition_profile(nutrition_data, takaran_saji):
    # PERUBAHAN Teks & Font: Profil Gizi & Makronutrien Holistik
    ...
    with col1:
        # PERUBAHAN Teks & Font: Kepadatan Energi (kkal/gram)
    ...
    with col2:
        # PERUBAHAN Teks & Font: Rasio Gula dari Total Karbohidrat
...
def render_health_metrics(nutrition_data, takaran_saji, current_threshold):
    # PERUBAHAN Teks & Font: Pemenuhan Angka Kecukupan Gizi Harian
...
if app_mode == "Analisis Produk Tunggal":
    # PERUBAHAN Teks & Font: Analisis Produk Tunggal
    ...
    with manual_input_col:
        # PERUBAHAN Teks & Font: Input Informasi Produk
    ...
    with manual_result_col:
        # PERUBAHAN Teks & Font: Hasil Analisis AI (Prediksi Risiko)
    ...
    with result_col:
        # PERUBAHAN Teks & Font: Hasil Analisis AI (Prediksi Risiko) (di bagian OCR)
        st.markdown("<h2 style='font-weight: 700; font-size: 1.5rem; color: #2C3E50; margin-bottom: 15px;'>Hasil Analisis AI (Prediksi Risiko)</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h1 style='font-weight: 800; font-size: 2.2rem; color: #1E3A8A; margin-bottom: 20px;'>Analisis Batch Excel</h1>", unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        ...
        # 1. Menampilkan dataframe input (Seperti pada gambar)
        st.dataframe(df, use_container_width=True)
        
        # 2. Menambahkan tombol aksi untuk memulai Batch
        if st.button("Mulai Analisis Batch", type="primary"):
            ...
            for idx, row in df_clean.iterrows():
                ...
                # Mendukung jika kolom nama produk menggunakan header 'Kemasan' dari OCR atau kolom lain
            ...
            # 4. Menyimpan hasil di Session State agar tidak hilang saat tombol download diklik
            st.session_state.batch_result_df = pd.DataFrame(results)
            st.session_state.batch_total_rows = total_rows
            
        # 5. Merender Hasil (Berdasarkan state session yang tersimpan)
        if st.session_state.batch_result_df is not None:
            ...
            st.markdown("<h2 style='font-weight: 700; font-size: 1.5rem; color: #2C3E50; margin-bottom: 15px; margin-top: 15px;'>Hasil Analisis Batch</h2>", unsafe_allow_html=True)
        ...
        # Bersihkan state jika pengguna menghapus file Excel
        st.session_state.batch_result_df = None
